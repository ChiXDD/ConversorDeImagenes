...
router.get('/', (req, res) => {
    res.render('convertirJPG', { titulo: 'Conversor a JPG'});
});
...
router.post('/', checkCredits, upload.array('conJPG', 100), async (req, res) => {
    if (req.user) {
        ...
        try {
            ...
            const conversions = req.files.map(async (file, index) => {
                const buffer = await sharp(file.buffer).toFormat('jpg').toBuffer();
                return { filename: `imagen_${index + 1}.jpg`, buffer };
            });
            ...
            if (req.files.length > 1) {
                ...
                const file = files[0];
                res.set('Content-Disposition', `attachment; filename="imagen_convertida_${Date.now()}.jpg"`);
