...
router.get('/', (req, res) => {
    res.render('convertirJPEG', { titulo: 'Conversor a JPEG'});
});
...
router.post('/', checkCredits, upload.array('conJPEG', 100), async (req, res) => {
    if (req.user) {
        ...
        try {
            ...
            const conversions = req.files.map(async (file, index) => {
                ...
                return { filename: `imagen_${index + 1}.jpeg`, buffer };
            });
            ...
            if (req.files.length > 1) {
                ...
                const file = files[0];
                res.set('Content-Disposition', `attachment; filename="imagen_convertida_${Date.now()}.jpeg"`);
