...
router.get('/', (req, res) => {
    res.render('convertirWEBP', { titulo: 'Conversor a WEBP'});
});

// Configuracion de multer para almacenar los archivo
const storage = multer.memoryStorage();
const upload = multer({ storage: storage });

// Ruta para la carga de archivos
router.post('/', checkCredits, upload.array('conWEBP', 100), async (req, res) => {
    if (req.user) {
        ...
        try {
            ...
            const conversions = req.files.map(async (file, index) => {
                ...
                return { filename: `imagen_${index + 1}.webp`, buffer };
            });
            ...
            if (req.files.length > 1) {
                ...
                const file = files[0];
                res.set('Content-Disposition', `attachment; filename="imagen_convertida_${Date.now()}.webp"`);
